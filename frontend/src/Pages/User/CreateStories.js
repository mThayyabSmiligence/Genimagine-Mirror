...
function CreateStories() {
  ...
  // FIXED: Updated handleCreateStory to reduce API calls
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        if (isEditMode) {
          ...
          if (autoGenerate) {
            // FIXED: For auto-generated stories, navigate with minimal state
            console.log('Auto-generation started, navigating to stories page...');
            navigate("/u/stories", { 
              replace: true,
              state: {
                newAutoStoryId: newStoryId, // Just pass the ID instead of full object
                justCreated: true,
                autoGenerated: true,
                message: `"${title}" generation started successfully!`
              }
            });
          } else {
