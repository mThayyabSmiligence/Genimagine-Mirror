...
export default function SubTopbar({setShowNavBar,showNavBar,width}) {
  ...
  return (
    <div className={`sub-top-bar ${showNavBar?"short":"big"} d-flex justify-content-between`} style={{width:`${showNavBar?width-250:width}px` }} >
        ...
        {
        loggedIn&&
        <div className='flex-1 d-flex align-items-center justify-content-end me-4'>
          
          <div className='d-flex profile-pic-small align-items-center justify-content-center'>
            <p className='m-0 mb-1'>{userData!=null&&userData.username[0]}</p></div>
        </div>
        }
    </div>
  )
